...
function main()
    ...
    # Periodic BC
    periodic_faces = collect_periodic_facets(grid, "Gamma1", "Gamma2", x -> x + Vec{2}((2*pi, 0.0)))
    pbc = PeriodicDirichlet(:u, periodic_faces)
    add!(ch, pbc)
    # Dirichlet BC
    dbc = Dirichlet(:u, union(getfacetset(grid, "Gamma"), getfacetset(grid, "Gammab")), x -> 0)
    add!(ch, dbc)
    close!(ch)
